...
pub mod helper {
    ...
    pub fn execute_pocklington_plan<F: PrimeField>(
        ...
        if !super::miller_rabin_prime::helper::miller_rabin_32b(&base_nat) {
            return None;
        }
...
mod test {
    ...
    #[test]
    fn mr_11() {
        assert_eq!(super::miller_rabin_prime::helper::miller_rabin(&BigUint::from(11usize), 3), true);
    }

    #[test]
    fn mr_12() {
        assert_eq!(super::miller_rabin_prime::helper::miller_rabin(&BigUint::from(12usize), 3), false);
    }

    #[test]
    fn mr_251() {
        assert_eq!(super::miller_rabin_prime::helper::miller_rabin(&BigUint::from(251usize), 11), true);
    }

    #[test]
    fn mr_15() {
        assert_eq!(super::miller_rabin_prime::helper::miller_rabin(&BigUint::from(15usize), 3), false);
    }
    ...
    impl<'a, E: PoseidonEngine<SBox = QuinticSBox<E>>> Circuit<E> for PrimeHash<'a, E> {
        fn synthesize<CS: ConstraintSystem<E>>(self, cs: &mut CS) -> Result<(), SynthesisError> {
            ...
            let (expected_ouput, _, _) =
                super::miller_rabin_prime::helper::hash_to_prime::<E>(&input_values, &domain, &self.params.hash)
                    .unwrap();
            let allocated_expected_output = BigNat::alloc_from_nat(
                cs.namespace(|| "output"),
                || Ok(expected_ouput),
